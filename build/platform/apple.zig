...
pub fn addArchive(b: *std.Build, ctx: *const common.Context) std.Build.LazyPath {
    ...
    for (slices.apple_slices) |slice| {
        ...
        create.addDirectoryArg(wrapFramework(b, buildSlice(b, ctx, slice), headers.getDirectory(), plist_file));
    }
...
fn wrapFramework(b: *std.Build, library: std.Build.LazyPath, headers: std.Build.LazyPath, plist: std.Build.LazyPath) std.Build.LazyPath {
