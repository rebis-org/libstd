...
fn buildSlice(b: *std.Build, ctx: *const common.Context, slice: slices.AppleSlice) std.Build.LazyPath {
    ...
    if (slice.arches.len == 1) return buildArch(b, ctx, base, slice.arches[0]);
    var libraries: [4]std.Build.LazyPath = undefined;
    for (slice.arches, 0..) |arch, index| libraries[index] = buildArch(b, ctx, b.fmt("{s}_{s}", .{ base, archName(arch.arch) }), arch);
    return universal(b, base, libraries[0..slice.arches.len]);
}

fn buildArch(b: *std.Build, ctx: *const common.Context, name: []const u8, arch: slices.AppleArch) std.Build.LazyPath {
    const target = b.resolveTargetQuery(targetQuery(arch));
    if (arch.zig) return common.addLibrary(b, target, .ReleaseFast, .dynamic, ctx).getEmittedBin();
